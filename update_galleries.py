...
<body>
  ...
  <script>
    ...
    function setupInfoButton(lgEl) {{
      lgEl.addEventListener('lgAfterOpen', () => {{
        ...
        if (toolbar && !toolbar.querySelector('.lg-info-wrap')) {{
          ...
          // Insert: [info] [fullscreen] [close]  — so insert before the fullscreen button
          const fsBtn = toolbar.querySelector('.lg-fullscreen');
          const closeBtn = toolbar.querySelector('.lg-close');
          const insertBefore = fsBtn || closeBtn;
          if (insertBefore) {{
            toolbar.insertBefore(infoWrap, insertBefore);
          }} else {{
    ...
    async function loadAlbumData() {{
      ...
      if (client && window.SUPABASE_CONFIG && !window.SUPABASE_CONFIG.SUPABASE_URL.includes('YOUR_SUPABASE')) {{
        try {{
          ...
          if (!error && data && data.length > 0) {{
            albumItems = data.map(r => ({{
              fullSrc: window.getPublicUrl(r.image_path),
              thumbSrc: window.getPublicUrl(r.thumbnail_path),
              title: r.title || '',
              subtitle: r.subtitle || '',
              ...
              exif: {{
                ...
                iso: r.iso ? String(r.iso) : null,
                focal: r.focal_length
              }}
            }}));
          }}
        }} catch (err) {{
          console.warn("Failed to fetch album from Supabase, trying images.json fallback:", err);
        }}
      }}
        ...
        try {{
          ...
          if (data.{album}) {{
            data.{album}.forEach(({{ file, caption, alt, exif }}) => {{
              ...
              let h2 = tempDiv.querySelector('h2');
              let h3 = tempDiv.querySelector('h3') || tempDiv.querySelector('h4');

              albumItems.push({{
                fullSrc:  `images/{album}/${{file}}`,
                thumbSrc: `images/{album}/thumbs/${{file}}`,
                title:    h2 ? h2.textContent : (caption && !caption.includes('<') ? caption : alt || file),
                subtitle: h3 ? h3.textContent : '',
                description: '',
                alt:      alt || "",
                exif:     exif || null
              }});
          ...
          console.error("Failed to load album images.json:", e);
        }}
      }}

      albumItems.forEach(({{ fullSrc, thumbSrc, title, subtitle, description, alt, exif }}) => {{
        const link = document.createElement("a");
        link.href = fullSrc;

        let titleHtml = title ? `<h2>${{title}}</h2>` : '';
        let subtitleHtml = subtitle ? `<h3>${{subtitle}}</h3>` : '';
        let descHtml = description ? `<p>${{description}}</p>` : '';
        const captionHtml = (titleHtml || subtitleHtml || descHtml) ? `<div class="lg-caption-wrap">${{titleHtml}}${{subtitleHtml}}${{descHtml}}</div>` : "";

        link.setAttribute("data-sub-html", captionHtml);
